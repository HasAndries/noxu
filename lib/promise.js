...
function runFailHandlers(promise, inputVal) {
  for (var ct = 0; ct < failHandlers.length; ct++) {
    failHandlers[ct].prev = promise;
    runPromise(failHandlers[ct], inputVal);
  }
}
...
Promise.prototype.reject = function (val) {
  ...
  else if (failHandlers.length) runFailHandlers(this, this.outputVal);
...
Promise.fail = function (handler) {
  failHandlers.push(anyToPromise(handler));
};
Promise.failRemoveAll = function(){
  failHandlers = [];
}
